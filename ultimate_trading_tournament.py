...
class TournamentConfig:
    ...
    # WATCHLIST
    # Note: AI teams can now choose from ALL portfolio stocks (all teams' positions) + watchlist
    # This allows teams to select from the entire portfolio, not just the watchlist
    ...
    CONFIG_HASH: str = field(default_factory=lambda: hashlib.md5(open(__file__, encoding='utf-8').read().encode('utf-8')).hexdigest()[:12])
...
class SmartFilter:
    ...
    def tier1_technical_screen(self, watchlist: List[str]) -> List[FilterResult]:
        ...
        print(f"\n🔍 TIER 1: Technical Screen ({len(watchlist)} stocks)")
        print("─" * 85)
        ...
        print(f"\n  ✅ {len(results)} stocks passed (Score ≥ {self.cfg.FILTER_MIN_SCORE})")
        return results
...
class AI_Team_Base:
    ...
    def get_relevant_memories_prompt(self, symbol: str, key_factors: List[str], 
        ...
        prompt = "\n\n📚 RELEVANT PAST TRADES (Learn from history):\n"
        prompt += "─" * 60 + "\n"
...
"""
        ...
        prompt += "\n💡 How does this inform your current decision?\n"
        return prompt
...
class TournamentDashboard:
    ...
    def generate_reviewer_accuracy_report(self, days: int = 30) -> str:
        ...
        report = f"""
📊 Reviewer Accuracy Report (Last {days} Days)
{'='*50}
...
class LearningEngine:
    ...
    def calibrate_reviewer(self):
        ...
        self.logger.info("🔄 Initiating reviewer retraining cycle...")
...
class RecursiveTournamentOrchestrator:
    ...
    def __init__(self, config: TournamentConfig):
        ...
        print("\n" + "="*90)
        print("🏆 ULTIMATE AI TRADING TOURNAMENT - KIMI API EDITION")
        print("="*90)
        print(f"📅 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"📊 Watchlist: {len(config.WATCHLIST)} stocks")
        print(f"👥 Active Teams: {len(self.teams)}")
        print(f"🔬 Experiment ID: {config.EXPERIMENT_ID}")
        print(f"📝 Config Hash: {config.CONFIG_HASH[:12]}...")
        print(f"📋 Rubric Version: {config.RUBRIC_VERSION}")
        for team_id, team in self.teams.items():
            print(f"   Team {team_id}: {team.team_name}")
        print(f"💰 Initial Capital per Team: ${config.INITIAL_VIRTUAL_CAPITAL:,.2f}")
        print("="*90)
    ...
    def run_daily_tournament(self):
        ...
        print("\n" + "="*90)
        print("🏁 DAILY TOURNAMENT CYCLE")
        print("="*90)
        
        self.update_all_prices()
        
        # Update market regime for each portfolio
        print("\n📊 STEP 1: Detect Market Regimes")
        print("─" * 85)
        for team_id, portfolio in self.portfolios.items():
            ...
            print(f"🤖 {portfolio.team_name}: Regime = {portfolio.current_regime.value}")
        
        # Portfolio review
        print("\n📊 STEP 2: Portfolio Review & Self-Critique")
        print("─" * 85)
        
        for team_id, portfolio in self.portfolios.items():
            if not portfolio.positions:
                print(f"\n🤖 {portfolio.team_name}: No open positions")
                continue
            
            print(f"\n🤖 {portfolio.team_name} - Reviewing {len(portfolio.positions)} positions:")
            ...
            for symbol, position in list(portfolio.positions.items()):
                ...
                if position.stop_loss and position.action in [TradeAction.BUY, TradeAction.STRONG_BUY]:
                    if current_price <= position.stop_loss:
                        ...
                        if critique:
                            print(f"  💭 Critique: {critique.critique_text[:80]}...")
                ...
                if position.target_price and position.action in [TradeAction.BUY, TradeAction.STRONG_BUY]:
                    if current_price >= position.target_price:
                        exit_type = ExitType.TARGET_HIT
                        print(f"  🎯 TARGET HIT: {symbol} @ ${current_price:.2f}")
                ...
                if days_held >= 30:
                    exit_type = ExitType.TIME_EXPIRED
                    print(f"  ⏱️  EXPIRED: {symbol} (held {days_held} days)")
                ...
                print(f"  📈 {symbol}: ${position.unrealized_pnl:+.2f} ({pnl_pct:+.1f}%)")
        
        # Find new opportunities
        print("\n🔍 STEP 3: Find New Opportunities")
        print("─" * 85)
        
        # Get all symbols from all portfolios (all teams can choose from whole portfolio)
        all_portfolio_symbols = list(set(
            [pos.symbol for portfolio in self.portfolios.values() for pos in portfolio.positions.values()]
        ))
        
        # Combine portfolio symbols with watchlist to ensure comprehensive coverage
        # This allows AI teams to choose from the entire portfolio, not just watchlist
        candidate_symbols = list(set(all_portfolio_symbols + self.cfg.WATCHLIST))
        
        print(f"📊 Screening from {len(candidate_symbols)} stocks:")
        print(f"   - Portfolio positions: {len(all_portfolio_symbols)} stocks")
        print(f"   - Watchlist: {len(self.cfg.WATCHLIST)} stocks")
        print(f"   - Total unique: {len(candidate_symbols)} stocks")
        
        screened = self.filter.tier1_technical_screen(candidate_symbols)
        if not screened:
            print("✅ No stocks passed screening today")
            return
        
        print(f"\n🎯 {len(screened)} stocks for AI analysis\n")
        
        # Get tournament state
        tournament_state = self.tournament_memory.get_tournament_state()
        
        # Review loop
        print("\n⚖️  STEP 4: Review Layer Evaluation")
        print("─" * 85)
        ...
        for team_id, team in self.teams.items():
            ...
            for stock in screened[:self.cfg.MAX_DAILY_ANALYSIS]:
                ...
                print(f"  🤖 Team: {rec.action.value} @ {rec.confidence}%")
                print(f"  ⚖️  Reviewer: {'✅ APPROVED' if review.approved else '❌ REJECTED'}")
                ...
                if review.approved:
                    ...
                    print(f"  ⛔ Trade blocked by reviewer")
                
                time.sleep(0.5)
            
            print(f"\n  📊 Reviewed: {reviewed_count} | Approved: {approved_count} | Rejected: {reviewed_count - approved_count}")
            print(f"  💰 API Cost: ${portfolio.api_costs:,.4f}")
        
        # Update portfolio values
        print("\n📈 STEP 5: Update Portfolio Values")
        print("─" * 85)
        ...
        for team_id, portfolio in self.portfolios.items():
            ...
            print(f"🤖 {portfolio.team_name}: ${portfolio.total_value:,.2f} ({portfolio.total_return_pct:+.1f}%)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="🏆 Ultimate AI Trading Tournament - Kimi Edition")
    ...
    if missing_teams and len(missing_teams) == len(config.ACTIVE_TEAMS):
        print("❌ ERROR: No valid API keys found for active teams!")
    ...
    try:
        ...
        for day in range(config.TOURNAMENT_DAYS):
            print(f"\n{'='*90}")
            print(f"📅 DAY {day + 1} / {config.TOURNAMENT_DAYS}")
            print(f"{'='*90}")
            ...
            if day < config.TOURNAMENT_DAYS - 1:
                print(f"\n⏳ Waiting for next day... (Ctrl+C to exit)")
                try:
        ...
        print(f"\n📄 Full report saved to: tournament_report_{config.EXPERIMENT_ID}.json")
